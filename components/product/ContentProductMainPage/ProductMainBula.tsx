// Arquivo: components/ProductBulaMain.tsx
import { Product } from "../../../commerce/ContentTypes.ts";

interface ProductBulaMainProps {
  contentPage: Product;
}

/**
 * Componente que renderiza a bula do produto principal em formato de acordeão
 * Similar ao ProductBula mas trabalha diretamente com o produto em vez da variação
 */
export function ProductBulaMain({ contentPage }: ProductBulaMainProps) {
  // Verifica se é um medicamento (produto do tipo 1) - verifica em ambos os locais possíveis
  const isProductMedicine = contentPage.productType === 1;

  // Se não for medicamento, não renderiza o componente
  if (!isProductMedicine) return null;

  // Verifica a existência de atributos do produto - agora diretamente do contentPage
  const productAttributes = contentPage.productAttribute;
  ...
  // Filtrar apenas o atributo "Interação Alimentícia" dos atributos da substância
  const substanceActionAttributeFoodInteraction =
    substance?.substanceAttribute?.find(
      (attr) => attr.attribute?.title === "Interação Alimentícia",
    ) || null;

  const result = substanceActionAttributeFoodInteraction
    ? {
      id: substanceActionAttributeFoodInteraction.id,
      productId: null,
      attributeId: substanceActionAttributeFoodInteraction.attributeId,
      attribute: substanceActionAttributeFoodInteraction.attribute,
      attributeValueId:
        substanceActionAttributeFoodInteraction.attributeValueId,
      attributeValue: null,
      value: substanceActionAttributeFoodInteraction.value,
    }
    : null;
  ...
  const visibleAttributes = combinedAttributes
    .filter(
      (attr) =>
        !attr?.attribute?.hidden && attr?.value && attr?.attribute?.title,
    )
    .sort((a, b) => {
  ...
  return (
    <div class="mb-8">
      <h2 class="self-stretch justify-center text-[#212529] text-[28px] font-medium font-['Inter'] leading-[33.60px] p-4 border-b border-[#dee2e6]">
        Bula do {contentPage.title}
      </h2>
      ...
      <div>
        {visibleAttributes.map((attr, index) => {
          ...
          return (
            <details key={index} class="group" open={index < 2}>
              <summary class="flex justify-between items-center p-4 cursor-pointer list-none border-b border-[#dee2e6] max-lg:px-2">
                <h3 class="self-stretch justify-center text-[#212529] text-2xl font-medium font-['Inter'] leading-[28.80px] lg:mr-8 mr-4">
                  {attr?.attribute?.customerTitle
                    ?.replace("{}", contentPage.title || "")
                    ?.replace("{name}", contentPage.title ?? "") ||
                    attr?.attribute?.title}
                </h3>
                <span class="text-[#212529] transition-transform duration-200 group-open:rotate-180 group-open:text-[#009999]">
                  <svg
                    width="20"
                    height="21"
                    viewBox="0 0 20 21"
              ...
              <div class="p-4 border-b border-[#dee2e6] max-lg:px-2">
                {isHtml
                  ? (
                    <div
        ...
        {contentPage.slug && (
          <div class="mt-6">
            <div class="flex flex-col md:flex-row gap-4 px-4 pt-2 pb-4">
              <a
                href={`/${contentPage.slug}/bula`}
                rel="noopener noreferrer"
                class="text-white text-lg p-3 hover:underline duration-300 rounded font-normal bg-primary hover:bg-accent"
              >
                Bula do {contentPage.title}
              </a>
            </div>
...
export default ProductBulaMain;
