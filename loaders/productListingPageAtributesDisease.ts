...
const productListingPageAtributes = async (
  ...
  const queryDisease =
    url.pathname?.replace(/\/doenca/, "")?.replace(/\//g, "") ?? "";

  const urlPromisse =
    `https://auxiliarapi.consultaremedios.com.br/Disease/_slug/${queryDisease}`;
  const headersDisease = {
    Authorization:
      "CR83A91E954D01B307D2F46D37BF39CB9B5179350E2787F48FC72C7CBA6BC76360",
    "Content-Type": "application/json",
  };

  const responseDisease = async () => {
    try {
      const response = await fetch(urlPromisse, {
        method: "GET",
        headers: headersDisease,
      });
  ...
  const disease = await responseDisease();
  ...
  const title = disease?.titleH1
    ? `${disease?.titleH1} com menor preço | CR`
    : `Remédios para ${disease?.diseaseName} com menor preço | CR`;
  const description = disease?.titleH1
    ? `${disease?.titleH1} com menor preço você encontra aqui. Compre online nas melhores farmácias e receba em casa via delivery!`
    : `Remédios para ${disease?.diseaseName} com menor preço você encontra aqui. Compre online nas melhores farmácias e receba em casa via delivery!`;
  ...
  return {
      ...
      itemListElement: [
        {
          ...
          name: "Doenças",
          position: 1,
          item: `${url.origin}/doencas`,
        },
        {
          "@type": "ListItem",
          name: disease?.titleH1?.replace("Remédios para ", "") ??
            disease?.diseaseName ??
            "",
          position: 2,
          item: `${url.origin}/doenca/${disease?.slug}`,
        },
      ],
    },
    seo: {
    ...
    titlePage: disease ? disease.titleH1 : null,
    contentSeo: disease?.description ?? null,
