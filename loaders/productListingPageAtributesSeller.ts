...
const dataSeller = [
  {
    name: "Economizar Comercio de Produtos Farmacêuticos Ltda Me",
    slug: "economizar-comercio-de-produtos-farmaceuticos-ltda-me",
  },
  {
    name: "Drogaria Nossa Farma - Pradopolis - Ltda",
    slug: "drogaria-nossa-farma-pradopolis-ltda",
  },
  {
    name: "Dimed S/a - Distribuidora de Medicamentos",
    slug: "dimed-sa-distribuidora-de-medicamentos",
  },
  {
    name: "Biomaxfarma Bias Fortes Ltda",
    slug: "biomaxfarma-bias-fortes-ltda",
  },
];
...
const productListingPageAtributes = async (
  ...
  const querySeller = dataSeller.find(
    (seller) => seller.slug == url.pathname.replace("/lojas/", ""),
  );
  ...
  const title = capitalize(querySeller?.name ?? "");
  const description = capitalize(query || "");
      ...
      itemListElement: [
        {
          ...
          name: "Lojas",
          position: 1,
          item: `${url.origin}/lojas`,
        },
        {
          "@type": "ListItem",
          name: querySeller?.name ?? "",
          position: 2,
          item: `${url.origin}/lojas/${querySeller?.slug}`,
        },
      ],
    },
    seo: {
