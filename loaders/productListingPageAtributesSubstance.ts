...
const productListingPageAtributes = async (
  ...
  const querySubstance = url.pathname
    ?.split("/")
    .filter(Boolean)
    .filter((segment) => segment !== "pa")
    .join("-");

  const substance = await getSubstance(querySubstance);
  ...
  const dataGenerics = await storefront.query<
    SearchQuery,
    SearchQueryVariables
  >({
    variables: {
      filters: [
        ...
        {
          field: "Tipo do medicamento",
          values: ["Similar Intercambiável"],
        },
        {
          field: "Tipo do medicamento",
          values: ["Similar irriseilaoque"],
        },
      ],
      query,
      operation: "AND",
  ...
  const productsGenerics = dataGenerics?.result?.productsByOffset?.items ?? [];
  ...
  const title =
    `${substance?.substanceName}: menor preço e entrega rápida, compre online | CR`;

  const description =
    `Medicamentos com ${substance?.substanceName} com menor preço e entrega 
rápida. Compre ${substance?.substanceName} através da Consulta Remédios e 
economize na farmácia!`;
  ...
  return {
      ...
      itemListElement: [
        {
          ...
          position: 1,
          item: `${url.origin}/principios-ativos`,
        },
        {
          "@type": "ListItem",
          name: substance?.substanceName ?? "",
          position: 2,
          item: `${url.origin}/${substance?.slug}/pa`,
        },
      ],
    },
    seo: {
    ...
    contentSeoSubstance: {
      substanceAttribute: substance?.substanceAttribute,
      nameSubstance: substance.substanceName,
      isSubstance: true,
      medicalSpecialty: substance?.medicalSpecialty,
      disease: substance?.disease,
      slug: substance?.slug,
      dcb: substance?.dcb,
      genericProducts: productsGenerics.length > 0
        ? productsGenerics?.map((item) => {
          return {
            href: item?.spotInformation ?? "",
            name: item?.productName ?? "",
          };
        })
        : null,
    } ?? null,
    titlePage: substance ? substance?.substanceName : null,
    products: products
