...
export interface Props {
  disease: ListingFromAtoZ;
}

function ListingFromAtoZ({ disease }: Props) {
  if (disease?.items.length < 1) {
    return <NotFound />;
  }

  const { pageInfo } = disease;
  ...
  const breadcrumbs = [
    ...
    {
      label: disease.paramsVowel,
      href: `/doencas/${disease.paramsVowel}`,
    },
  ];
  ...
  return (
    <section class="max-w-[1336px] lg:px-5 m-auto py-6 lg:py-8">
        ...
        <div>
          <div>
            <h1 class="text-[1.75rem] font-medium tracking-[-.09375rem] leading-tight text-info pt-6 max-lg:px-4">
              Doenças com a letra:{" "}
              <span class="uppercase">{disease.paramsVowel}</span>
      ...
      {disease.items.length && (
        <Pagination
          pageInfo={pageInfo}
          pathPrevius={`/doencas/${disease.paramsVowel}`}
        />
      )}
    </section>
  );
...
export const loader = (props: Props, _req: Request, ctx: AppContext) => {
  if (props.disease?.items.length < 1) {
