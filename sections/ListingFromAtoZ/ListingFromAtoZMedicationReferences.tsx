...
export interface Props {
  naveguete?: ListAtoZ;
  medicationReferences: ListingFromAtoZresponse;
}

function ListingFromAtoZ({ naveguete, medicationReferences }: Props) {
  return (
      ...
      <div>
        ...
        <ul class="flex bg-white gap-2 py-6 max-lg:overflow-x-auto sticky top-0 border-b border-neutral max-lg:px-4">
          {naveguete?.list?.map((list) => (
            <li>
              <a
                title={`Medicamentos Referência com a letra: ${list?.label}`}
        ...
        <div>
          {naveguete?.list?.map((list) => {
            const filteredClasses = medicationReferences?.items?.filter(
              (medicationReferences) => {
                const label = list?.label?.toLowerCase() || "";
                const medicationReferencesName =
                  medicationReferences.title?.toLowerCase() || "";

                if (label === "0-9") {
                  // Cenário 1: label é "0-9", traz todos os nomes que começam com números
                  return /^\d/.test(medicationReferencesName);
                } else {
                  // Cenário 2: verifica se começa com a mesma string do label
                  return medicationReferencesName.startsWith(label);
                }
              },
            );
            return (
              <div>
                <h2 class="text-[1.75rem] font-medium tracking-[-.09375rem] leading-tight text-info pt-6 max-lg:px-4">
                  Medicamentos de Referência com a letra:{" "}
                ...
                <ul class="grid grid-cols-1 lg:grid-cols-2 gap-x-6 py-6">
                  {filteredClasses
                    ?.slice(0, 20)
                    ?.map((medicationReferences) => (
                      <li class="border-b border-neutral py-4">
                        <a
                          title={medicationReferences?.customerTitle ??
                            medicationReferences?.title ??
                            ""}
                          href={`/${medicationReferences.slug}/p`}
                ...
                <div class="flex w-full justify-center pb-4 border-b border-neutral">
                  <a
                    ...
                    href={`/medicamentos-referencia/${list?.label}`}
