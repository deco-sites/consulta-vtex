...
function ListingFromAtoZ({ medicines }: Props) {
  ...
  const breadcrumbs = [
    { label: "home", href: "/" },
    { label: "Bulas", href: "/bulas" },
    {
      label: medicines.paramsVowel,
      href: `/bulas/${medicines.paramsVowel}`,
    },
  ];
  ...
  return (
    <section class="max-w-[1336px] lg:px-5 m-auto py-6 lg:py-8">
      ...
      {medicines?.items?.length && (
        <Pagination
          pageInfo={pageInfo}
          pathPrevius={`/bulas/${medicines.paramsVowel}`}
        />
      )}
    </section>
  );
