...
export interface Props {
  naveguete?: ListAtoZ;
  factory: ListingFromAtoZresponse;
}

function ListingFromAtoZ({ naveguete, factory }: Props) {
  return (
      ...
      <div>
        ...
        <ul class="flex bg-white gap-2 py-6 max-lg:overflow-x-auto sticky top-0 border-b border-neutral max-lg:px-4">
          {naveguete?.list?.map((list) => (
            <li>
              <a
                title={`Fabricantes com a Letra: ${list?.label?.toLocaleUpperCase()}`}
        ...
        <div>
          {naveguete?.list?.map((list) => {
            const filteredClasses = factory?.items?.filter((factory) => {
              const label = list?.label?.toLowerCase() || "";
              const factoryName = factory.factoryName?.toLowerCase() || "";

              if (label === "0-9") {
                // Cenário 1: label é "0-9", traz todos os nomes que começam com números
                return /^\d/.test(factoryName);
              } else {
                // Cenário 2: verifica se começa com a mesma string do label
                return factoryName.startsWith(label);
              }
            });
            return (
              <div>
                ...
                <ul class="grid grid-cols-1 lg:grid-cols-2 gap-x-6 py-6">
                  {filteredClasses?.slice(0, 20)?.map((factory) => (
                    <li class="border-b border-neutral py-4">
                      <a
                        title={factory?.customerTitle ??
                          factory.commercialName ?? ""}
                        href={`/fabricante/${factory.slug}`}
