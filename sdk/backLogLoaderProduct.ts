...
export default async function ProductDetailsPage(
  ...
  console.log(
    productContent?.wakeProductVariantId,
    productContent?.wakeProductId,
  );
  ...
  async function searchProductVariants(
    ...
    while (attempt < maxAttempts) {
      ...
      const productResult = items.find(
        (item) =>
          item.spotInformation?.replace(
  ...
  const { productId, productResult } = await searchProductVariants(
    product,
    slug,
    validateSlug,
  );
